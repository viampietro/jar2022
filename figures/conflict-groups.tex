...
\begin{tikzpicture}
  ...
  \node[transition] (t1) [below right=6mm of p0, label={left:$t_1$}] {}
  edge[test, bend right] (p0);

  \node[transition] (t3) [below=3mm of p0, label={left:$t_3$}] {}
  edge[pre] (p0);
  
  \node[place, tokens=1] (p1) [right=1cm of p0, label={above:$p_1$}] {};
  \node[transition] (t2) [below right=6mm of p1, label={left:$t_2$}] {}
  edge[inhib, bend right] (p1);
  \draw (t1) edge[pre, bend left] (p1);

  \node[transition] (t4) [below=3mm of p1, label={left:$t_4$}] {}
  edge[pre] (p1);
