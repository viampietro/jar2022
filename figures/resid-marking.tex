...
\begin{tikzpicture}
  ...
  \node (pn1) {
    \begin{tikzpicture}
      ...
      \node[transition, anchor=north east] (t0) at ($(p0.south west)-(.5,.5)$) [label={left:$\mathbf{t_0}$}] {}
      edge[pre, bend left] (p0);

      \node[transition, anchor=north west] (t2) at ($(p0.south east)+(.5,-.5)$) [label={left:$t_2$}] {}
      edge[pre, bend right] (p0);

      \node[transition] (t1) at ($(t0)!.5!(t2)$) {}
      edge[pre] (p0);
      \node[anchor=east] at ($(t1.west)+(0,.1)$) {
  ...
  \node (pn3) at ($(pn2.east)+(2,0)$) {
    \begin{tikzpicture}
      ...
      \node[transition, anchor=north west] (t2) at ($(p0.south east)+(.5,-.5)$) [label={left:$\mathbf{t_2}$}] {}
      edge[pre, bend right] (p0);
