...
% engineering, models are first-order concepts in MBSE.
...
input model in a way that would alter its behavior. % Thus,
...
% neuroprostheses. % This software is currently used by the engineers of the Neurinnov company to design the digital systems having a part in safety-critical implantable medical devices.  To the purpose of formal verification, we will implement the \hilecop{} model-to-text transformation leveraging the functional language of the \coq{} proof assistant. However, after the mechanization of the proof of semantic preservation, we could use the extraction feature of the \coq{} proof assistant to produce the implemented transformation as an \ocaml{} program. We will then be able to connect this program to the existing \hilecop{} software in order
...
% when the upper bound is infinite. In
% Premise~\ref{it:locked-counters}, the \emph{locked} time counters
% refer to the time counters that have overreached the upper bound of
% their associated time interval. Of course, time counters can never
% be locked in the presence of an infinite upper bound.
...
\begin{figure}[H]
  ...
  \caption[Double consumption of token in a SITPN.]{Double consumption
    ...
    % marking resulting from the firing of $t_0$ and $t_1$.
...
\begin{figure}[H]
  ...
  \label{fig:mutex}
\end{figure}
In Situation \circled{1} of Figure~\ref{fig:mutex}, condition $c_1$ is
associated to $t_1$ and the complementary condition is associated to
$t_0$ thus creating the mutual exclusion. In Situation \circled{2},
...
% , and more particularly the ones used in the pre-defined place and
% transition designs.
...
\begin{table}[!h]
  \resizebox{\textwidth}{!}{%  
    \begin{tabular}{l|l}
      ...
      \quad\quad\vhdle|o$_1$ : out std_logic;|
      & \quad\quad$(\mathtt{out}, \mathtt{o}_1, \mathtt{bool}),$ \\
      \quad\quad\vhdle|o$_2$ : out std_logic;|
      & \quad\quad$(\mathtt{out}, \mathtt{o}_2, \mathtt{bool})$ \\
      \quad\vhdle|);| & \quad$\},$ \\
      \vhdle|end nand_gate;| & \\
      & \\
...
\begin{table}[!h]
  ...
  \begin{prooftree}[template=\inserttext]
    ...
    \infer[no rule]1{$\mathcal{D},E_p,\Delta,\tau_c,\sigma_0\vdash{}{}d.beh\rightarrow\theta$}
    
    \infer2 [$\forall\tau_i\in\mathbb{N},~\mathsf{dom}(E_p)(\tau_i)\subseteq{}\mathsf{dom}(I(\Delta))$] { $\mathcal{D},\mathcal{M}_g,E_p,\tau_c\vdash$
...
% COMMAND RUN TO COUNT PROG, SPECS AND PROOF LINES:
% 
% coqwc $(find sitpn/ -name *.v ! -regex "sitpn/proofs/.*")
%
...
% COMMAND RUN TO COUNT PROG, SPECS AND PROOF LINES:
% 
% coqwc $(find hvhdl/ -name *.v ! -regex "hvhdl/proofs/.*")
%
...
% COMMAND RUN TO COUNT PROG, SPECS AND PROOF LINES:
% 
% coqwc $(find transformation/ -name *.v ! -regex "transformation/proofs/.*")
%
...
% COMMAND RUN TO COUNT PROG, SPECS AND PROOF LINES:
% 
% coqwc $(find common/ sitpn/proofs/ hvhdl/proofs/ transformation/proofs/ behavior-preservation/ -name *.v)
%
...
% the theorems and lemmas that we proved rely on the well-definedness
...
% enables the encapsulation of a SITPN subnet, called a refinement,
...
%     double-lined circle that encapsulates a SITPN subnet; the subnet
