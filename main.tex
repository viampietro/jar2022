...
% These operations will influence the behavior described by the SITPN
% model through the interpretation elements (i.e. conditions, actions
% and functions).

% During the transformation from Step 1 to Step 2
% (cf. Figure~\ref{fig:hilecop-wf}), the elements connected through a
% fusion arc will be merged together.

% A component has an interface that exposes elements of its internal
% behavior to the outside. An SITPN model defines the internal behavior
% of a given component.  Inside the component, the engineer can handle
% \vhdl{} signals and perform operations over them, for instance to
% write to an output port.

% These will be translated into \vhdl{} signals during the model-to-text
% transformation.

% As presented in Figure~\ref{fig:hilecop-wf}, the
% transformation from Step 1 to Step 2
