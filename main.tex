...
structural and behavioral description of hardware circuits.
...
matter of fact, no transitions are fired during the first rising
...
multiple execution step in the target program leads from state $t$ to
...
know as today. It represents no more than 100000 lines of HOL4
code. However, no information is given regarding the number of persons
that worked on the project, and during how much time to mechanized the
whole.  In \cite{Chlipala2010}, A. Chlipala verifies a compiler from
an impure functional language to some abstract low-level assembly
...
possible to express the transformation in a mdoular way. First, we
...
\todo[inline]{Add concrete metrics about the mechanization (lines of
  formalization, lines of proof scripts, person-years, etc.)}
