...
well-definition of a SITPN input model, which are required to express
...
events of a clock signal with the state transition relation of
Definition~\ref{def:semantics}. In Definition~\ref{def:semantics} and
in the remainder of the article, note that the sum expression $\sum$
will contextually corresponds to the Boolean sum expression, i.e. the
application of the \texttt{or} operator over the elements of the
iterated set, or to the sum between natural numbers. Each definition
case of the state transition relation is the conjunction of the
enumerated items.
